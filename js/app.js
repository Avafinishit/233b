...
function scrollChatToBottomNow() {
    ...
    if (chatBox) {
        const overflows = chatBox.scrollHeight > chatBox.clientHeight + 4;
        chatBox.scrollTop = overflows ? chatBox.scrollHeight : 0;
    }
...
const FORUM_IMAGE_WINDOW_SIZE = 5;
const FORUM_IMAGE_MAX_PER_WINDOW = 2;
const ADMIN_TOPUP_STORAGE_KEY = 'adminTopup99999Applied_20260513';
const ADMIN_TOPUP_AMOUNT = 99999;
const WALLET_TEST_MONEY_CLEARED_STORAGE_KEY = 'walletTestMoneyCleared_20260518';
const WALLET_TEST_BALANCE_THRESHOLD = 50000;
const LEGACY_DEFAULT_WALLET_BALANCE = 1000;
...
function loadChatHistory() {
    ...
    const cleanedHistory = stripTransientTypingNoticesFromHistory(chatHistory);
...
function isForumScreenImagePost(post) {
    const text = getForumPostImageText(post);
    return /(屏幕|截图|截屏|桌面|电脑|显示器|监控|字幕|二维码|扫码|screen|screenshot|desktop|monitor|display)/i.test(text);
}

function isForumPhoneImagePost(post) {
    const text = getForumPostImageText(post);
    return /(手机|聊天记录|聊天截图|私信截图|消息截图|通话记录|未接来电|来电|短信|微信|QQ|app|APP|应用|壁纸|锁屏|通知栏|手机壳|充电|电量|验证码|朋友圈|社交软件|mobile|phone|smartphone|chat log|message screenshot|wallpaper|app\b|notification)/i.test(text);
}
...
function getForumImageTopicGuidance(forum) {
    ...
    if (isCatForumName(forumName)) {
        return 'Cat-forum visual anchor: prefer cats and cat-life details such as a real cat by a window, food bowl, litter box, cat carrier, vet visit detail, scratched sofa, cat toys, fur on clothes, paw prints, cat bed, water fountain, or a candid owner-perspective cat photo. Do not turn cat posts into generic device screenshots.';
    }

    return forumName
        ? `Forum topic anchor: the image should visibly belong to a "${forumName}" community when possible, using concrete subjects from that topic instead of generic devices.`
        : '';
}

function removeForumPhoneDeviceBiasFromPrompt(prompt = '') {
    return String(prompt || '')
        .replace(/\b(?:smartphone|mobile phone|cell phone|phone|handheld phone|phone screen|mobile screen|mobile app|messaging app|phone camera|mobile photography|app screenshot|chat UI|mobile UI)\b/gi, 'everyday object')
        .replace(/\b(?:screen showing|screen displaying|display showing|displaying a forum post|showing a forum post|forum post screenshot|forum screenshot)\b/gi, 'real-world scene about')
        .replace(/\b(?:device UI|forum post|social media post)\b/gi, 'physical subject')
        .replace(/\b(?:iPhone|Android)\b/gi, 'device')
        .replace(/手机|智能手机|手机屏幕|手机镜头|手机摄影|移动端|移动应用|聊天界面|消息界面|论坛截图|帖子截图|屏幕显示|屏幕上|应用截图/g, '日常物件')
        .replace(/手里拿着日常物件|hand holding an everyday object/gi, 'nearby everyday object')
        .replace(/\s{2,}/g, ' ')
        .trim();
}
...
function inferForumPostImageKind(post) {
    ...
    if (/(聊天记录|聊天截图|私信截图|私信|已读不回|消息截图|通话记录|短信截图|通知栏截图|微信聊天|QQ聊天|聊天框|对话框|message screenshot|chat log)/i.test(text)) {
        return 'chat';
    }
...
function stripExactPostTextFromImagePrompt(prompt = '', post = null) {
    ...
    forbidden.forEach((phrase) => {
        result = result.replace(new RegExp(escapeRegExpLiteral(phrase), 'g'), '');
    });

    return result
        .replace(/Post title:[^\n]*\n?/gi, '')
        .replace(/Post body:[\s\S]*?(?=\n[A-Z][A-Za-z ]+:|\n[A-Z][a-z]+:|$)/g, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim();
}

function hasForumPostImageSlot(post) {
    return !!(
        post?.imageUrl
        || ['queued', 'generating', 'processing'].includes(String(post?.imageStatus || '').toLowerCase())
    );
}

function canQueueForumPostImageWithinWindow(post, sortedPosts = []) {
    if (!post?.id) return false;
    const index = sortedPosts.findIndex(item => String(item.id) === String(post.id));
    if (index < 0) return true;

    const windowStart = Math.floor(index / FORUM_IMAGE_WINDOW_SIZE) * FORUM_IMAGE_WINDOW_SIZE;
    const windowPosts = sortedPosts.slice(windowStart, windowStart + FORUM_IMAGE_WINDOW_SIZE);
    const imageCount = windowPosts.filter(item => String(item.id) !== String(post.id) && hasForumPostImageSlot(item)).length;
    return imageCount < FORUM_IMAGE_MAX_PER_WINDOW;
}

function getForumPostImageCandidates(forumId, posts = []) {
    const forum = getForumById(forumId);
    if (!forum) return [];
    const sortedPosts = (forum.posts || [])
        .slice()
        .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    const batchSize = Math.max(1, posts.length || 1);
    const maxImages = Math.min(
        Math.ceil(batchSize / FORUM_IMAGE_WINDOW_SIZE) * FORUM_IMAGE_MAX_PER_WINDOW,
        Math.max(1, Math.min(batchSize, FORUM_IMAGE_MAX_PER_WINDOW))
    );

    return maxImages > 0
        ? posts
            .filter(shouldGenerateForumPostImage)
            .filter(post => canQueueForumPostImageWithinWindow(post, sortedPosts))
...
function wrapForumImagePrompt(basePrompt, post, forum) {
    ...
    const allowPhone = isForumPhoneImagePost(post);
    const allowScreen = isForumScreenImagePost(post) || allowPhone;
    const sourcePrompt = allowPhone
        ? stripExactPostTextFromImagePrompt(basePrompt, post)
        : removeForumPhoneDeviceBiasFromPrompt(stripExactPostTextFromImagePrompt(basePrompt, post));
    const topicGuidance = getForumImageTopicGuidance(forum);
    ...
    return [
        ...
        sourcePrompt,
        topicGuidance,
        world ? `World/context for mood only; do not render this context as visible text: ${world}.` : '',
        ...
        allowPhone
            ? 'Phone rule: a phone, app UI, or phone screen is allowed here only because the post explicitly mentions phones, apps, wallpaper, messages, calls, or mobile UI.'
            : 'Phone rule: do NOT include any phone, smartphone, phone screen, mobile UI, chat UI, app screenshot, or hand holding a phone.',
        allowScreen && !allowPhone
            ? 'Screen rule: if a screen is needed, use a computer monitor, TV, printed photo, sign, or other non-phone display that fits the post.'
            : '',
        !allowScreen ? 'Screen rule: avoid screens unless a non-screen real-world subject would be impossible.' : '',
        'The final prompt intentionally omits the exact post title/body; infer the visual idea from the scene description only.',
...
function buildForumPostImagePrompt(post, forum) {
    ...
    if (aiPrompt && !(isGamePost && promptLooksLikeChatScreenshot)) {
        return wrapForumImagePrompt([
            ...
            `Seed idea: ${allowPhone ? aiPrompt : removeForumPhoneDeviceBiasFromPrompt(aiPrompt)}.`,
    ...
    if (isGamePost) {
        return wrapForumImagePrompt([
            ...
            'Scene: a casual desk setup after a match, computer monitor or TV showing game-like results/statistics, keyboard or controller nearby, dim room light, slightly imperfect handheld camera photo.',
    ...
    if (imageKind === 'text_object') {
        return wrapForumImagePrompt([
            ...
            allowPhone
                ? 'Scene: book pages, a notebook, sticky note, menu, paper document, computer screen, or phone screen, depending on what the post explicitly implies.'
                : 'Scene: book pages, a notebook, sticky note, menu, paper document, sign, receipt, package label, or handwritten note. Do not use a phone or app screen.',
            ...
            'Composition: close-up or over-the-shoulder candid photo, believable lighting, shallow depth of field, mild real-life imperfection.',
    ...
    if (imageKind === 'scene') {
        return wrapForumImagePrompt([
            ...
            'Composition: candid everyday photography, realistic lighting, ordinary framing, slightly imperfect but pleasant.',
    ...
    return wrapForumImagePrompt([
        ...
        'Possible direction: a desk detail, a room corner, an object mentioned in the post, a street or commute scene, a pet/object/food/weather detail, or a quiet environmental photo that reflects the mood.',
        'Composition: candid everyday photography, realistic lighting, natural colors, ordinary framing, believable imperfections.',
...
- Do not default to phones. A phone should appear only if the post explicitly concerns chat screenshots, phone screens, apps, wallpapers, calls, messages, notifications, or mobile UI.
- If the forum is about cats, choose cat-life imagery first: real cats, cat supplies, litter box, vet carrier, scratched sofa, food bowl, window perch, toys, fur, paw details, or candid owner-perspective cat moments.
...
- Do not include the exact post title or exact post body in your final prompt, even as "context". Convert them into visual ideas instead.
- If text appears, make it short, newly invented, context-appropriate, or partially obscured. Avoid walls of text.
- Prefer candid everyday photography with natural light and believable framing unless the post clearly asks for another style.
...
function buildForumImagePromptDirectorUserPrompt(post, forum) {
    ...
    const allowPhone = isForumPhoneImagePost(post);
    const allowScreen = isForumScreenImagePost(post) || allowPhone;
    const displayedSeedPrompt = allowPhone ? seedPrompt : removeForumPhoneDeviceBiasFromPrompt(seedPrompt);

    return `Forum name: ${forum?.name || 'local community'}
World/context: ${world || 'none'}
Inferred image category: ${imageKind}
Phone allowed: ${allowPhone ? 'yes' : 'no'}
Non-phone screen allowed: ${allowScreen && !allowPhone ? 'yes' : allowPhone ? 'yes, but only if a phone is truly the most natural subject' : 'no'}
...
5. ${allowPhone
        ? 'Uses a phone only if it is the most natural subject for this explicit phone/app/wallpaper/message/call post.'
        : 'Does not include any phone, smartphone, phone screen, mobile UI, app screenshot, chat UI, or hand holding a phone.'}
6. ${allowScreen && !allowPhone
        ? 'If a screen is needed, use a computer monitor, TV, printed photo, sign, or other non-phone display.'
        : 'Prefer a non-screen real-world subject unless the post truly requires a display.'}
7. Feels like a believable image a normal forum user would attach.`;
}
...
function generateForumImagesForPosts(forumId, posts = []) {
    ...
    if (!batchKey) return;
    const candidates = getForumPostImageCandidates(forumId, posts);
...
function buildFallbackForumTemplatePool(forum, options = {}) {
    ...
    const forumName = String(forum?.name || '').trim();
    const isCatForum = isCatForumName(forumName);
    ...
    return [
        ...
        [`突然好奇大家桌面都摆了什么`, `刷帖刷累了，想换个心情。有没有那种看着很舒服、但又不是刻意布置的角落。`],
        [`你们觉得熬夜和早起哪个更伤身体`, `最近作息乱到离谱，想听听真实体验，不要养生号那种复制粘贴。`],
        [`有没有人推荐舒服点的白噪音`, `夜里太安静反而睡不着，雨声和风扇声都可以，别太像广告。`],
...
function isForumPostOnTopic(post, forum) {
    const forumName = String(forum?.name || '').trim();
    if (!forumName) return true;
    ...
    if (normalizedName.length >= 2) {
        const normalizedText = normalizeForumComparableText(text);
        return normalizedText.includes(normalizedName) || getForumTextSimilarity(normalizedText, normalizedName) > 0.08;
    }

    return true;
}

async function improveInitialForumPostComments(forumId, posts = []) {
    const forum = getForumById(forumId);
    if (!forum || !Array.isArray(posts) || posts.length === 0) return;

    let changed = false;
    for (const postRef of posts) {
        const post = getForumPostById(forumId, postRef.id);
        if (!post) continue;
        post.comments = Array.isArray(post.comments) ? post.comments : [];
        if (post.comments.length > 0 && !areForumCommentsLowQuality(post.comments, post)) continue;

        try {
            const generated = post.comments.length > 0
                ? await requestForumAICommentsRewrite(forum, post, post.comments, '')
                : await requestForumAIComments(forum, post, '');
            const source = generated.length && !areForumCommentsLowQuality(generated, post)
                ? generated
                : [];
            if (!source.length) continue;

            const baseTime = Number(post.createdAt) || Date.now();
            const comments = uniquifyForumCommentAuthors(source
                .map((comment, index) => normalizeForumComment({
                    ...
                    createdAt: getForumCommentCreatedAt(baseTime, index, source.length)
                }))
                .filter(comment => comment.content && !isLowQualityForumCommentText(comment.content, post))
                .slice(0, 4), [], forum);
    ...
    if (changed) {
        ...
        saveForums();
        if (String(currentForumId) === String(forumId)) renderForumDetail();
        if (String(currentForumId) === String(forumId) && currentApp === 'forum-post') renderForumPostDetail();
    }
}
...
async function appendGeneratedForumPosts(forumId = currentForumId, options = {}) {
    ...
    try {
        ...
        let posts = dedupeForumPosts(
            normalizeGeneratedForumPosts(rawPosts, forum, options).filter(post => isForumPostOnTopic(post, forum)),
            forum.posts || []
        );
        ...
        if (posts.length < Math.max(1, expectedCount)) {
            posts = [
                ...
                }).filter(post => isForumPostOnTopic(post, forum))
            ];
        }
        ...
        getForumPostImageCandidates(forum.id, posts).forEach(post => {
            if (!post.imageStatus) post.imageStatus = 'queued';
            post.imageError = '';
        });
        forum.updatedAt = Date.now();
        saveForums();
        renderForumDetail();
        improveInitialForumPostComments(forum.id, posts);
        generateForumImagesForPosts(forum.id, posts);
    } finally {
...
function shouldShowForumPostLoadingThumb(post) {
    if (!post || post.imageUrl) return false;
    const status = String(post.imageStatus || '').toLowerCase();
    if (['queued', 'generating', 'processing'].includes(status)) return true;
    if (status === 'failed') return false;
    return !!String(post.imagePrompt || '').trim();
}
...
function renderForumPostRow(post, showHotTag = false) {
    ...
    const hotTag = showHotTag
        ...
        : '';
    const imagePreview = renderForumPostThumb(post);
    const meta = [
...
function openForumPublishSheet() {
    ...
    overlay.innerHTML = `
        <div class="forum-publish-sheet">
            ...
            <div class="forum-sheet-title">想发点什么？</div>
...
function tokenizeForumContentForQuality(text = '') {
...
function isLowQualityForumCommentText(content = '', post = null) {
    ...
    if (post) {
        const postTokens = tokenizeForumContentForQuality(`${post.title || ''} ${post.content || ''}`)
            .filter(token => !['这个', '那个', '感觉', '不是', '真的', '有点', '大家', '评论', '论坛'].includes(token));
        if (postTokens.length >= 4) {
            const matched = postTokens.filter(token => text.includes(token)).length;
            if (matched === 0 && text.length < 24) return true;
        }
    }
...
function areForumCommentsLowQuality(comments = [], post = null) {
    const valid = comments
        .map(comment => String(comment?.content || '').trim())
        .filter(Boolean);

    if (valid.length === 0) return true;
    if (valid.every(text => text.length < 14)) return true;
    if (valid.filter(text => isLowQualityForumCommentText(text, post)).length >= Math.ceil(valid.length / 2)) return true;
    return false;
}
...
async function appendForumAutoComments(forumId, postId, userComment = '') {
    ...
    try {
        ...
        if (areForumCommentsLowQuality(rawComments, post)) {
            const rewrittenComments = await requestForumAICommentsRewrite(forum, post, rawComments, userComment);
            if (rewrittenComments.length && !areForumCommentsLowQuality(rewrittenComments, post)) {
                rawComments = rewrittenComments;
            }
        }
    } catch (error) {
...
function applyAdminWalletTopupOnce() {
    // Legacy test top-up is intentionally disabled.
}

function isAdminWalletTopupRecord(record = {}) {
    const id = String(record?.id || '');
    const title = String(record?.title || record?.name || '');
    const note = String(record?.note || '');
    const amount = Number(record?.amount) || 0;
    return id.startsWith('admin_topup_')
        || title === '后台加款'
        || note === '后台加款'
        || amount === ADMIN_TOPUP_AMOUNT;
}

function removeLegacyWalletTestMoney() {
    if (localStorage.getItem(WALLET_TEST_MONEY_CLEARED_STORAGE_KEY) === 'true') {
        localStorage.setItem(ADMIN_TOPUP_STORAGE_KEY, 'true');
        return;
    }

    const hadLegacyAdminTopup = localStorage.getItem(ADMIN_TOPUP_STORAGE_KEY) === 'true';
    const records = Array.isArray(walletData.records) ? walletData.records : [];
    const filteredRecords = records.filter(record => !isAdminWalletTopupRecord(record));
    const removedAmount = records.reduce((sum, record) => (
        isAdminWalletTopupRecord(record) ? sum + (Number(record?.amount) || 0) : sum
    ), 0);

    let nextBalance = Number(walletData.balance) || 0;
    if (removedAmount > 0) {
        nextBalance = Math.max(0, nextBalance - removedAmount);
    }
    if (
        hadLegacyAdminTopup
        || nextBalance >= WALLET_TEST_BALANCE_THRESHOLD
        || nextBalance === LEGACY_DEFAULT_WALLET_BALANCE
    ) {
        nextBalance = DEFAULT_WALLET_BALANCE;
    }

    const changed = filteredRecords.length !== records.length || Number(walletData.balance) !== nextBalance;
    if (changed) {
        walletData = {
            balance: Number(formatTransferAmount(nextBalance)),
    ...
    localStorage.setItem(ADMIN_TOPUP_STORAGE_KEY, 'true');
    localStorage.setItem(WALLET_TEST_MONEY_CLEARED_STORAGE_KEY, 'true');
}
...
function getLoveLetterMysteryReward() {
    return MYSTERY_SHOP_REWARDS.find(reward => reward.id === 'love-letter') || {
        id: 'love-letter',
        name: '情书',
        image: '',
        description: '一封来自角色的手写情书'
...
function normalizeMysteryPurchaseReward(purchase = {}) {
    if (String(purchase?.itemId || '') !== 'mystery') return purchase;
    const reward = getLoveLetterMysteryReward();
    return {
        ...purchase,
        reward: {
            ...reward,
            revealedAt: purchase?.reward?.revealedAt || purchase?.timestamp || Date.now()
        },
        giftedRewardId: purchase?.giftedRewardId && purchase.giftedRewardId !== 'love-letter'
            ? 'love-letter'
            : purchase?.giftedRewardId
    };
}

function loadShopData() {
    shopData = normalizeShopData(safeReadStorageJSON(SHOP_STORAGE_KEY, {}));
    const originalPurchases = Array.isArray(shopData.purchases) ? shopData.purchases : [];
    const purchases = originalPurchases.map(normalizeMysteryPurchaseReward);
    const mysteryReward = shopData.mysteryReward
        ? {
            ...getLoveLetterMysteryReward(),
            revealedAt: shopData.mysteryReward.revealedAt || Date.now()
        }
        : null;
    const changed = purchases.some((purchase, index) => {
        const original = originalPurchases[index];
        return String(original?.reward?.id || '') !== String(purchase?.reward?.id || '')
            || String(original?.giftedRewardId || '') !== String(purchase?.giftedRewardId || '');
    }) || String(shopData.mysteryReward?.id || '') !== String(mysteryReward?.id || '');
    shopData = {
        purchases,
        mysteryReward
    };
...
function pickMysteryShopReward() {
    return getLoveLetterMysteryReward();
}
...
function getAvatarRawImageUrl(avatar) {
    const normalizedAvatar = getSoftAvatarColorValue(avatar);
    if (isDataImageUrl(normalizedAvatar) || /^https?:\/\//i.test(normalizedAvatar) || isMediaRef(normalizedAvatar)) {
        return normalizedAvatar;
    }

    const urlMatch = String(normalizedAvatar || '').match(/url\((['"]?)(.*?)\1\)/i);
    return urlMatch?.[2]?.trim() || '';
}

function getAvatarImageCssValue(avatar) {
    const rawUrl = getAvatarRawImageUrl(avatar);
    if (!rawUrl || isMediaRef(rawUrl)) return '';
    return `url('${rawUrl.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}')`;
}

function isUnresolvedAvatarMediaRef(avatar) {
    return isMediaRef(getAvatarRawImageUrl(avatar));
}

function getAvatarRenderConfig(avatar, nickname = '?') {
    const normalizedAvatar = getSoftAvatarColorValue(avatar);
    const fallbackText = getAvatarFallbackText(nickname);
    const imageCssValue = getAvatarImageCssValue(normalizedAvatar);
    ...
    if (imageCssValue) {
        return {
            avatarContent: '',
            avatarStyle: `background-image: ${imageCssValue}; background-size: cover; background-position: center; background-repeat: no-repeat; border: 0; color: transparent; text-shadow: none;`
        };
    }

    if (isWhiteAvatar || isUnresolvedAvatarMediaRef(normalizedAvatar)) {
        return {
...
async function hydrateAvatarValueForRender(avatar) {
    const rawUrl = getAvatarRawImageUrl(avatar);
    if (!isMediaRef(rawUrl)) return avatar;

    const dataUrl = await resolveMediaRefToDataUrl(rawUrl);
...
async function renderContactsList() {
    ...
    const contactRows = await Promise.all(contacts.map(async (contact) => {
        ...
        const hydratedAvatar = await hydrateAvatarValueForRender(contact.avatar);
        const avatarConfig = getAvatarRenderConfig(hydratedAvatar, contact.nickname);
...
function stripTransientTypingNoticesFromHistory(history = []) {
    ...
    return history.filter((message) => {
        ...
        return !isTransientTypingNoticeText(message.content);
    });
}

function removeTransientTypingNoticeNodes(root = document) {
    root.querySelectorAll?.('.chat-system-notice, .msg-bubble-ai.system, #loadingMsg').forEach((node) => {
        if (node.id === 'loadingMsg' || isTransientTypingNoticeText(node.textContent)) {
            node.remove();
        }
    });
}
...
function renderChatMessageAvatar(element, avatar, nickname = '?', options = {}) {
    ...
    if (isUrlAvatar) {
        ...
        const imageCssValue = getAvatarImageCssValue(avatar);
        if (imageCssValue) {
            element.style.setProperty('background-image', imageCssValue, 'important');
...
3. 线上回复优先短句，可以1~5条气泡感短句。允许只回“？”“行”“你干嘛”这类极短反应；单独一个“？”可以是一整条完整回复，代表惊讶、质疑、无语或让对方自己品。感叹号不要单独成条，只适合跟“啊”“哎呀”“不是吧”这类短反应一起出现。
4. 允许半句话、碎句、没说满的句子、自然重复口癖、语气词、问号、省略号和轻微打断感；比起总结信息，更像正在聊天。线上少用完整长句和正式标点，能拆开就拆开。
...
10. 生活反应贴合“性格”。冷淡的人少撒娇，毒舌的人会损两句，温柔的人会短短关心，黏人的人才多说想念和亲昵称呼；每个角色保留自己的亲密方式。
...
async function retryAICall(userText, role, chatBox, previousPrompt, options = {}) {
    ...
    try {
        const modeWarning = isOfflineMode
            ...
            : '3. 线上模式：像手机聊天，可以1~5条短气泡，也可以只发一个“？”当完整回复；感叹号不要单独成条。允许半句、碎句、口癖和轻微打断感。';
...
function showAIError(text) {
    ...
    if (!chatBox) return;
    removeTransientTypingNoticeNodes(chatBox);
    const errorMsg = createChatSystemNotice(text);
    errorMsg.classList.add('chat-system-error');
...
function waitMusicPlaybackRetry(ms = MUSIC_PLAYBACK_RETRY_DELAY_MS) {
...
function prepareMusicAudioForUserGesture() {
    bindMusicAudio();
    const audio = getMusicAudio();
    if (!audio) return;

    const previousSrc = audio.getAttribute('src') || '';
    const previousCurrentTime = Number(audio.currentTime) || 0;
    const previousMuted = audio.muted;
    const restoreAudio = () => {
        if (audio.getAttribute('src') !== MUSIC_AUDIO_UNLOCK_SRC) {
    ...
    audio.src = MUSIC_AUDIO_UNLOCK_SRC;
    audio.muted = true;
...
function resetMusicPlaybackRetry() {
    musicPlaybackRetryToken += 1;
}
...
function clearProfileImageCaches() {
    // Avatar images are user profile data, not transient chat image cache.
    return false;
}
...
async function renderWechatChatList() {
    ...
    const roleRows = await Promise.all(wechatRoles.map(async (role) => {
        ...
        const avatarBaseStyle = 'width: 50px; height: 50px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 560; line-height: 1; text-align: center; position: relative;';
        const hydratedAvatar = await hydrateAvatarValueForRender(role.avatar);
        const avatarConfig = getAvatarRenderConfig(hydratedAvatar, role.nickname);
        const avatarStyle = `${avatarBaseStyle} ${avatarConfig.avatarStyle}`;
    ...
    chatList.innerHTML = roleRows.join('');
}
