...
let crcTable = null;
const NOVELESS_BLOCKED_BOOK_TITLE_PATTERNS = [
    /8716574/i,
    /\u652f\u4ed8\u5b9d/i,
    /\u4f59\u989d\u7ea2\u5305/i
];
...
async function searchNovelessBooks(query) {
    ...
    while ((itemMatch = itemRegex.exec(xml)) !== null) {
        ...
        if (isBlockedNovelessBookTitle(cleanTitle)) continue;
...
function parseBookListFromHtml(html) {
    ...
    while ((match = titleRegex.exec(html)) !== null) {
        ...
        if (isBlockedNovelessBookTitle(title)) continue;
...
function isBlockedNovelessBookTitle(title = '') {
    const value = String(title || '').replace(/\s+/g, '');
    return NOVELESS_BLOCKED_BOOK_TITLE_PATTERNS.some(pattern => pattern.test(value));
}
...
function bookstoreCorsHeaders(extra = {}) {
    return {
...
function bookstoreJsonResponse(statusCode, payload) {
    return {
        statusCode,
        headers: bookstoreCorsHeaders({
...
async function handleBookstoreRequest({ path = '', query = {}, method = 'GET' }) {
    if (method === 'OPTIONS') {
        return {
            statusCode: 204,
            headers: bookstoreCorsHeaders(),
    ...
    if (method !== 'GET') {
        return bookstoreJsonResponse(405, { error: 'Method Not Allowed' });
    }

    const pathname = String(path || '').split('?')[0];
    const action = String(query.action || pathname.split('/').filter(Boolean).pop() || '').trim();

    try {
        if (action === 'search') {
            return bookstoreJsonResponse(200, await searchNovelessBooks(query.q));
        }

        if (action === 'categories') {
            return bookstoreJsonResponse(200, await getNovelessCategories());
        }

        if (action === 'category') {
            return bookstoreJsonResponse(200, await getNovelessCategoryBooks(query.slug, query.page));
        }

        if (action === 'download') {
            const bookInfo = await prepareNovelessBook(query.id);
            return bookstoreJsonResponse(200, {
                ...bookInfo,
        ...
        if (action === 'chunk') {
            return bookstoreJsonResponse(200, await getNovelessChunk(query.id, query.index));
        }

        return bookstoreJsonResponse(404, { error: 'Unknown bookstore action' });
    } catch (error) {
        return bookstoreJsonResponse(error.statusCode || 500, {
            error: error.message || 'Bookstore request failed'
        });
    }
}
