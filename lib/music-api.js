...
const TOUBIEC_API_ORIGIN = 'https://nextmusic.toubiec.cn';
...
function requestJsonPostToUrl(targetUrl, payload = {}, headers = {}) {
    return new Promise((resolve, reject) => {
        const requestModule = targetUrl.protocol === 'http:' ? http : https;
...
async function requestToubiecSessionKey() {
    const apiUrl = new URL('/api/key', TOUBIEC_API_ORIGIN);
...
async function requestToubiecApi(pathname, payload = {}) {
    const session = await requestToubiecSessionKey();
    const apiUrl = new URL(pathname, TOUBIEC_API_ORIGIN);
...
async function resolveMusic163SongViaToubiec(id, options = {}) {
    ...
    const infoPayload = await requestToubiecApi('/api/getSongInfo', { id: normalizedId });
    if (Number(infoPayload?.code) === 429) throw new Error(infoPayload.message || 'Music parser rate limit');
    if (Number(infoPayload?.code) !== 200 || !infoPayload?.data) return null;
    ...
    const track = createMusic163TrackFromToubiecInfo(infoPayload.data, normalizedId);
...
async function resolveMusic163SongById(id, options = {}) {
    ...
    const strictToubiec = !allowMetadataOnly && options.strictToubiec !== false;
...
async function searchMusic163Songs(keyword, options = {}) {
    ...
    const limit = Math.min(30, Math.max(1, Number(options.limit) || 20));
    const offset = Math.max(0, Number(options.offset) || 0);
    const strictToubiec = options.strictToubiec !== false;
...
async function handleMusic163Action(action, query) {
    if (action === 'resolve') {
        ...
        const allowFallback = ['1', 'true', 'legacy', 'fallback'].includes(String(query.fallback || '').trim().toLowerCase());
        const song = await resolveMusic163SongById(id, { preferXfabe, strictToubiec: !allowFallback });
        return song
