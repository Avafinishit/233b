...
const TOUBIEC_INFO_TIMEOUT_MS = 650;
...
async function resolveMusic163SongViaToubiec(id, options = {}) {
    ...
    if (cached?.song && cached.expiresAt > Date.now()) return cached.song;
...
async function resolveMusic163SongById(id, options = {}) {
    ...
    const allowMetadataOnly = Boolean(options.allowMetadataOnly);
    const strictToubiec = !allowMetadataOnly;
    ...
    try {
        ...
        metadataTrack = detailById.get(normalizedId) || null;
    } catch (error) {
        console.warn('Reading music metadata failed:', error?.message || error);
    }

    return normalizeMusic163PlaylistSong(
        metadataTrack || { id: normalizedId, name: `Music ${normalizedId}`, artists: [], album: {}, duration: 0 },
        '',
...
async function handleMusic163Action(action, query) {
    if (action === 'resolve') {
        ...
        const song = await resolveMusic163SongById(id, { strictToubiec: true });
        return song
