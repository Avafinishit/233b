...
function requestDirectStream(targetUrl, options) {
  const requestModule = targetUrl.protocol === "http:" ? http : https;
  const requestOptions = {
    protocol: targetUrl.protocol,
    hostname: targetUrl.hostname,
...
function requestHttpStreamThroughProxy(targetUrl, proxyUrl, options) {
  const requestModule = proxyUrl.protocol === "https:" ? https : http;
  const headers = {
    ...(options.headers || {}),
    Host: targetUrl.host
...
async function requestHttpsStreamThroughProxy(targetUrl, proxyUrl, options) {
  const timeoutMs = options.timeoutMs || DEFAULT_TIMEOUT_MS;
  const tlsSocket = await connectHttpsThroughProxy(targetUrl, proxyUrl, timeoutMs);
  const agent = new https.Agent({
    keepAlive: false,
...
async function requestText(url, options = {}) {
  ...
  try {
    if (targetUrl.protocol === "http:") {
      return await requestHttpThroughProxy(targetUrl, proxyUrl, options);
    }

    if (targetUrl.protocol === "https:") {
      return await requestHttpsThroughProxy(targetUrl, proxyUrl, options);
    }
  } catch (error) {
    if (isLoopbackHost(proxyUrl.hostname) && isProxyConnectionError(error)) {
      console.warn("[upstream-request] local proxy failed, retrying direct:", {
...
async function requestStream(url, options = {}) {
  ...
  if (!proxyUrlText || shouldBypassProxy(targetUrl, proxyUrlText)) {
    return requestDirectStream(targetUrl, options);
  }

  const proxyUrl = new URL(proxyUrlText);
  try {
    if (targetUrl.protocol === "http:") {
      return await requestHttpStreamThroughProxy(targetUrl, proxyUrl, options);
    }

    if (targetUrl.protocol === "https:") {
      return await requestHttpsStreamThroughProxy(targetUrl, proxyUrl, options);
    }
  } catch (error) {
    if (isLoopbackHost(proxyUrl.hostname) && isProxyConnectionError(error)) {
      console.warn("[upstream-request] local proxy failed, retrying direct stream:", {
        proxy: proxyUrl.toString(),
        target: targetUrl.hostname,
        code: error?.cause?.code || error?.code || "",
        message: error?.message || "Unknown error"
